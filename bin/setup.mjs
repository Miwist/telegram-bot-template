...
async function main() {
  ...
  console.log(
    chalk.gray(
      "ℹ Этот шаблон создаст бота на TypeScript с готовой структурой проекта\n"
    )
  );
  ...
  if (!projectName) {
    ...
    console.log(
      `  npx grammy-bot-template ${chalk.yellow("мой-телеграм-бот")}`
    );
    process.exit(1);
  }
  ...
  try {
    ...
    if (err.code === "EEXIST") {
      console.error(
        chalk.red(`❌ Ошибка: папка "${projectName}" уже существует!`)
      );
  ...
  await copyDir(path.join(__dirname, "../template"), projectPath);
  ...
  const userPackageJson = JSON.parse(
    await fs.readFile(userPackageJsonPath, "utf-8")
  );
  ...
  console.log(chalk.cyan(`\n3. Доступные команды:`));
  console.log(
    `   ${chalk.yellow("npm start")}      - запуск бота в режиме разработки`
  );
  console.log(`   ${chalk.yellow("npm run build")}  - сборка проекта`);
  console.log(
    `   ${chalk.yellow(
      "npm run dev"
    )}    - запуск с hot-reload (если установлен nodemon)`
  );
